                ...
                <div class="ms-2 ps-3 d-flex">
                    <div class="position-relative">
                        <img src="./img/icons8-heart-32.png"
                             alt="">
                        <span class="span1 position-absolute
                                                text-white countere counter interests-count">{{auth()->check()?\App\Models\Interest::all()->count():\App\Http\Headers\Interest\Interest::all()->count()}}</span>
                    </div>
            ...
            @foreach(auth()->user()->interests as $interest)
                <div class="col-12 col-lg-3 p-0 m-0" id="interest-{{$loop->index}}">
                    <div class="card cd1r1 me-3">
                        ...
                        <!-- img in card1 -->
                        <a href="{{route('single_product',$interest->interestable->id)}}">
                            <img class="card-img-top
                                        img-fluid cdimgtop"
                                 src="{{!$interest->interestable->images()->count()==0?url($interest->interestable->images->image):''}}"
                                 alt="Card image cap">
            ...
            @foreach(\App\Http\Headers\Interest\Interest::all() as $interest)
                <div class="col-12 col-lg-3 p-0 m-0" id="interest1-{{$loop->index}}">
                    <div class="card cd1r1 me-3">
                        ...
                        <!-- img in card1 -->
                        <a href="{{route('single_product',$interest['product']->id)}}">
                            <img class="card-img-top
                                        img-fluid cdimgtop"
                                 src="{{!$interest['product']->images()->count()==0?url($interest['product']->images->image):''}}"
                                 alt="Card image cap">
...
<div class="navformobile d-flex flex-column border position-fixed
    ...
    <div class="row">
        ...
        <div class="col-3 text-center position-relative"><a href="#"><img src="./img/icons8-heart-32.png" alt=""style="width:25px; height=25px">
                <span class="span1
                                                    position-absolute
                                                    text-white counter1 interests-count1"id="counter1">{{auth()->check()?\App\Models\Interest::all()->count():\App\Http\Headers\Interest\Interest::all()->count()}}</span>
            </a> </div>
    ...
    function delete_interest(event,id,index){
        $.ajax({
            ...
            success:function (result){
                ...
                document.getElementById('interest-'+index).remove()
                $('.interests-count').html($('.interests-count').html()-1);
                $('.interests-count1').html($('.interests-count1').html()-1);
            }
        });
    }
    function delete_interest1(event,id,index){
