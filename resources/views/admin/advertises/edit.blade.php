    ...
    <div class="row">
        <div class="col-lg-12">
            <div class="card card">
                ...
                <form class="form-horizontal" method="post" action="{{route('admin.advertises.update',$advertise->id)}}" enctype="multipart/form-data">
                    @csrf
                    @method('patch')
                    <div class="card-body">
                        <div class="form-group">
                            ...
                            <div class="col-sm-10">
                                <input type="text" name="title" class="form-control" value="{{old('title',$advertise->title)}}" id="inputEmail3" placeholder="عنوان تبلیغ را وارد کنید">
                            </div>

                        </div>
                        <label>انتخاب تصویر</label>
                        <div class="input-group">
                            <input type="text" value="{{old('image',$advertise->image)}}" id="image_label" class="form-control" name="image"
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control" name="products[]" id="products" multiple>
                                @foreach(\App\Models\Product::all() as $product)
                                    <option value="{{ $product->id }}" {{in_array($product->id,$advertise->products->pluck('id')->toArray())?'selected':''}}>{{ $product->title }}</option>
                                @endforeach
                            </select>
                        </div>
